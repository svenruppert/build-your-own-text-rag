 ...
 * the zero-based indices of the chunks the model actually cited,
 * a refusal flag, an optional grounding check result, and the total
 * latency from the start of generation to the end of the stream.
 *
 * @param text                full answer text after the stream has ended
 * @param citedChunkIndices   zero-based indices into {@code usedHits} that
 *                            the answer references via {@code [Chunk N]}
 *                            (note: the answer text uses 1-based numbers
 *                            to match the prompt's chunk labels)
...
public record GeneratedAnswer(
    ...
    public GeneratedAnswer withGrounding(GroundingResult grounding) {
        ...
        return new GeneratedAnswer(
                ...
                refusalDetected, Optional.of(grounding), latencyMillis);
    }
}
